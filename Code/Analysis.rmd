...
# load the data
df <- read_csv("Amazon_categories_final_clean.csv")
...
# now for 1 star
df_Rating %>% 
  ggplot(aes(x = GPT_estimated_cleaned, y = perc_1_star, fill = Brand)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Percentage of 1 star rating per category",
       x = "Category",
       y = "Percentage of 1 star rating") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
